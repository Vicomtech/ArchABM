...
class PropagationModel:
    ...
    def rna_in_aerosol(self, respiratory_fluid_RNA_conc, mean_wet_aerosol_diameter):
        return (
            ...
            * math.pi
            / 6
            * (mean_wet_aerosol_diameter / 10000) ** 3
        )
    ...
    def aerosol_emission(
        ...
        return (
            (
                emission_breathing * (1 - speaking_breathing_ratio)
                + emission_speaking * speaking_breathing_ratio
            )
            * 1000
            * respiratory_rate
            * 60
    ...
    def dosis_six_hours(
        ...
        total_mask_efficiency_exhaling_inhaling,
    ):
        return (
            rna_dosis
            / (room_ventilation_rate + 1 / virus_lifetime_in_aerosol)
            * (1 - total_mask_efficiency_exhaling_inhaling)
            * 6
        )
    ...
    def risk_one_person(
        self, infection_probability, dosis_infectious, susceptible_people
    ):
        return (
            1 - ((1 - infection_probability) ** dosis_infectious) ** susceptible_people
        ) * 100
    ...
    def calculate_risk(
        ...
        rna_in_aerosol = self.rna_in_aerosol(
            respiratory_fluid_RNA_conc, mean_wet_aerosol_diameter
        )  # 3.27E-02

        aerosol_emission = self.aerosol_emission(
            emission_breathing,
            emission_speaking,
            speaking_breathing_ratio,
            respiratory_rate,
        ...
        aerosol_conc = self.aerosol_conc(
            aerosol_emission, room_area, room_height
        )  # 0.3800

        rna_cont = self.rna_cont(aerosol_conc, rna_in_aerosol)  # 0.0124

        rna_dosis = self.rna_dosis(
            respiratory_rate, rna_cont, deposition_probability
        )  # 3.7306

        dosis_six_hours = self.dosis_six_hours(
            rna_dosis,
            room_ventilation_rate,
            virus_lifetime_in_aerosol,
            total_mask_efficiency,
        ...
        dosis_infectious = self.dosis_infectious(
            dosis_six_hours, time_in_room_h
        )  # 47.7

        individual_infection_risk = self.individual_infection_risk(
            infection_prob, dosis_infectious
        )  # 9.9

        risk_one_person = self.risk_one_person(
            infection_prob, dosis_infectious, susceptible_people,
        )  # 91.9
