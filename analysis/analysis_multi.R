...
places %>% 
  dplyr::group_by(name, activity, id) %>% 
  dplyr::summarise(num_events=dplyr::n(), .groups = "keep") %>% 
  dplyr::ungroup() %>% 
  ggplot2::ggplot()+
  ggplot2::geom_col(ggplot2::aes(x=name, y=num_events, fill=activity, group=factor(id)),
                    position="dodge", alpha=0.9, width=0.6)+
...
# REMOVE
people %>% 
  dplyr::add_count(name, activity, id, name = "count") %>% 
  # dplyr::mutate(repeat_max = ifelse(is.na(repeat_max), max(count), repeat_max)) %>% 
  ggplot2::ggplot()+
  ggplot2::geom_jitter(ggplot2::aes(x=activity, y=count), width = 0.1, height=0.2, alpha=0.1)+
  ...
  ggplot2::facet_grid(rows=dplyr::vars(id)) +
...
people %>% 
  ...
  ggplot2::ggplot()+
  ggridges::geom_density_ridges(ggplot2::aes(x=date, y=activity, fill=factor(id)),
                                color=NA, alpha=0.1, bandwidth=15*60)+
  # ggplot2::scale_fill_manual(values=palette)+
  ggplot2::scale_x_datetime(date_breaks = "hour", date_labels = "%H:%M")+
  ggplot2::theme_bw()+
  ...
  ggplot2::labs(x="Date", y="Activity", fill="Simulation")
...
people %>% 
  ...
  ggplot2::facet_grid(cols=dplyr::vars(id)) +
  ggplot2::scale_color_manual(values=palette)+
  ggplot2::scale_x_datetime(date_breaks = "hour", date_labels = "%H:%M")+
  ggplot2::theme_bw()+
...
people %>% 
  ...
  dplyr::mutate(mean_risk = mean(final_risk),
                sd_risk = sd(final_risk)) %>% 
  dplyr::ungroup() %>% 
  ggplot2::ggplot()+
  # ggplot2::geom_hline(ggplot2::aes(yintercept=mean_risk), color="blue", size=0.5, linetype="solid")+
  ggplot2::geom_rug(ggplot2::aes(y=mean_risk), alpha=0.1, size=0.1, outside=F, sides="tr")+
  ggplot2::coord_cartesian(clip = "off")+
  # ggplot2::geom_hline(ggplot2::aes(yintercept=mean_risk+sd_risk), color="blue", size=0.5, linetype="dashed")+
  # ggplot2::geom_hline(ggplot2::aes(yintercept=mean_risk-sd_risk), color="blue", size=0.5, linetype="dashed")+
  ggplot2::geom_line(ggplot2::aes(x=person_id, y=final_risk, group=run), alpha=0.2, na.rm=T)+
  # ggplot2::geom_point(ggplot2::aes(x=person_id, y=final_risk), alpha=0.5, na.rm=T)+
  ggplot2::theme_bw()+
...
places %>% 
  ...
  ggplot2::ggplot()+
  ggplot2::geom_line(ggplot2::aes(x=date, y=air_quality, color=activity, group=factor(run)), alpha=0.1)+
  # ggplot2::geom_point(ggplot2::aes(x=date, y=air_quality, color=activity, group=name), alpha=0.1)+
